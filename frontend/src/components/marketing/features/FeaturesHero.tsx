...
function EcosystemWave() {
  ...
  useEffect(() => {
    ...
    const prepareHidden = (el: SVGPathElement) => {
      const length = el.getTotalLength();
      el.style.transition = 'none';
      el.style.strokeDasharray = `8 ${length}`;
      el.style.strokeDashoffset = `${length}`;
    };

    const drawSegment = (el: SVGPathElement) =>
      new Promise<void>((resolve) => {
        ...
        requestAnimationFrame(() => {
          el.style.transition = `stroke-dashoffset ${SEGMENT_MS}ms cubic-bezier(0.4, 0, 0.2, 1)`;
          el.style.strokeDashoffset = '0';
        });
        timers.push(window.setTimeout(() => resolve(), SEGMENT_MS));
      });
    ...
    const run = async () => {
      ...
      segs.forEach(prepareHidden);
      setStarted(true);

      // Start at first dot, draw to next, reveal text, repeat
      setActive(0);
      await drawSegment(segs[0]);
      setActive(1);
      await new Promise<void>((r) => {
        timers.push(window.setTimeout(() => r(), PAUSE_MS));
      });
      await drawSegment(segs[1]);
      setActive(2);
    };
        ...
        >
          {SEGMENTS.map((d, i) => (
            <path
              ...
              strokeLinecap="round"
              strokeWidth="3"
              style={{ strokeDasharray: '8 1200', strokeDashoffset: 1200 }}
            />
          ))}
          {([50, 500, 950] as const).map((cx, i) => (
            <circle
              key={cx}
              cx={cx}
              cy={100}
